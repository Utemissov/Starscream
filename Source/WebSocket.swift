...
public class WebSocket: NSObject, NSStreamDelegate {
    ...
    public func writeString(str: String, completion: (() -> ())? = nil) {
        ...
        writeData(str.dataUsingEncoding(NSUTF8StringEncoding)!, completion: completion)
    }
    ...
    public func writeData(data: NSData, completion: (() -> ())? = nil) {
        ...
        if data.length <= frameLength{
            ...
            while offset < data.length {
                ...
                isFin = offset + frameLength >= data.length
                offset += frameLength
                length = min(frameLength, data.length - offset)
                let payload = data.subdataWithRange(range)
                dequeueWrite(payload, code: offset <= frameLength ? .BinaryFrame : .ContinueFrame, isFin: isFin, writeCompletion: nil)
            }
        }
    }
    ...
    private func dequeueWrite(data: NSData, code: OpCode, isFin: Bool = true, writeCompletion: (() -> ())? = nil) {
        writeQueue.addOperationWithBlock { [weak self] in
            ...
            buffer[0] = (isFin ? s.FinMask : 0x00) | code.rawValue
            if dataLength < 126 {
